...
else
{
	...
	$db->connect();
	$sql = "SELECT * FROM `useraccount` WHERE `first_name` = '".$firstname."' AND `last_name`= '".$lastname."'";
	...
	else
	{
		...
		$sql = "INSERT INTO useraccount (password,first_name,last_name,gender,birthday,join_date,email,phone) VALUES";
		$sql .="('$password','$firstname','$lastname','$gender','$birthday','$jd','$email','$phone')";
		$result = mysql_query($sql);
		if(!$result)
		{
		...
		echo "<script>";
		echo "alert('Registrion complete!');";
		echo "window.location.href=\"magazines.php\";";
		echo "</script>";
